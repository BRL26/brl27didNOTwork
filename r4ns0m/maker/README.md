...
[Ably](https://ably.com) back to your PC, which then runs a jumpscare on your
desktop: the **A-90 style jumpscare** (from "The Ransom") or a **Foxy
fullscreen video** — whichever `trigger_switch.txt` selects.
...
Ably channel "rpc-trigger"
        │ WebSocket
        ▼
pc_client.py (on your PC)  ──►  action.sh  ──►  jumpscare.py   (switch = 1)
        ▲                                     └─►  foxy_jumpscare.py  (else)
...
| `action.sh` | Called by pc_client when a trigger arrives; picks the jumpscare. |
| `trigger_switch.txt` | Pick which jumpscare runs: `1` → A-90, else Foxy. |
| `jumpscare.py` | The A-90 jumpscare (window, stages, static, shake). |
| `foxy_jumpscare.py` | The Foxy jumpscare (plays `asset2/foxy.webm` fullscreen once). |
| `site/` | The GitHub Pages site the Discord button opens (publishes the trigger). |
| `asset/` | Images + audio the A-90 jumpscare uses. |
| `asset2/` | The Foxy jumpscare video + audio (`foxy.webm`, `foxy.mp3`). |
...
Clicking the Discord button fires `action.sh`. It reads `trigger_switch.txt`
and picks the first `0` or `1` digit it finds (other characters are ignored):

- **`1`** → runs `jumpscare.py`, the A-90 scare:
...
- **anything else (or a missing file)** → runs `foxy_jumpscare.py`: a plain
  fullscreen playback of `asset2/foxy.webm` (played once, aspect-ratio
  preserved) with `asset2/foxy.mp3` underneath if present.

ESC anytime quits early. A-90 stage windows are backgrounded with a screenshot
of the desktop so they are not black boxes.
...
- `foxy.webm` — the Foxy jumpscare video (frames are extracted with `ffmpeg`
  at 10 fps on first run, cached in `/tmp/rpc_foxy_frames`).
- `foxy.mp3` — optional audio; plays once and holds the scare until it ends.
